...
def is_legal_plan(city_map, building):
    h_ok = (min(building['used_h']) >= 0) and (max(building['used_h']) < city_map.shape[0])
    w_ok = (min(building['used_w']) >= 0) and (max(building['used_w']) < city_map.shape[1])
